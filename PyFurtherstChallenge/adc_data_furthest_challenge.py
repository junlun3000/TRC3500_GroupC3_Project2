...
READ_BYTES = 5
COLLECT_SECONDS = 3
SAVE_BASE_DIR = "./data_furthest_challenge"  # Base directory

# === Ensure base directory exists ===
os.makedirs(SAVE_BASE_DIR, exist_ok=True)

# === Menu Options ===
object_types = ['env', 'eraser']
...
def select_object():
    ...
    for idx, obj in enumerate(object_types):
        print(f"{idx}: {obj}")
    obj_choice = int(input("Enter choice: "))
    current_object = object_types[obj_choice]
...
def collect_data(object_type):
    dir_path = os.path.join(SAVE_BASE_DIR, object_type)
    os.makedirs(dir_path, exist_ok=True)

    existing_files = [f for f in os.listdir(dir_path) if f.startswith("adc_") and f.endswith(".data")]
    file_number = len(existing_files) + 1

    filename = os.path.join(dir_path, f"adc_{object_type}_{file_number}.data")
