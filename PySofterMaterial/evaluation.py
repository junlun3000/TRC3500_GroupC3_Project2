from fft_extract_softer_material import extract_features_from_adc, get_features_row
import serial
import os
import time
...
FILENAME = "evaluation_softer_material.data"
...
def determine_output(label):
    if label == 0:
        return "distance of 10cm and height of 10cm"
    elif label == 1:
        return "distance of 30cm and height of 10cm"
    elif label == 2:
        return "distance of 10cm and height of 30cm"
    elif label == 3:
        return "distance of 30cm and height of 30cm"
...
def predict(filename):
    ...
    scaler: StandardScaler = joblib.load("./MLP_training_softer_material/scaler.joblib")
    ...
    # print(model.device)
    model.load_state_dict(torch.load("./MLP_training_softer_material/best_model.pth", map_location="cpu"))
    model.eval()
    with torch.no_grad():
