...
def determine_label(object_name, height_cm, distance_cm):
    if height_cm == 10 and distance_cm == 10:
        return 0
    elif height_cm == 10 and distance_cm == 30:
        return 1
    elif height_cm == 30 and distance_cm == 10:
        return 2
    elif height_cm == 30 and distance_cm == 30:
        return 3
    else:
...
def get_features_row(file_path, combined_features, eval=False):
    ...
    if not eval:
        ...
        if 'soft_material' in parts:
            obj = 'soft_material'
        else:
            return None

        height_part = [p for p in parts if 'height' in p][0]
        distance_part = [p for p in parts if 'distance' in p][0]
        height_cm = int(height_part.replace('height','').replace('cm',''))
        distance_cm = int(distance_part.replace('distance','').replace('cm',''))

        label = determine_label(obj, height_cm, distance_cm)
...
if __name__ == "__main__":
    dataset_dir = './data_softer_material'   # 顶层目录
    output_csv_path = './features_output_softer_material.csv'  # 输出路径
    all_feature_rows = []
